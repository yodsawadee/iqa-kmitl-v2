...
def kNN(training_set, test_set):
    ...
    y_train = training_set.iloc[:,37].values
    ...
    #print(y_pred)
    test_set.iloc[:,37] = y_pred
    
    return test_set[['Prof_Primary_ID','Class']]


def viewAllNewRecomendedCommittee(variable1):

    train = pd.read_excel("study_program/training set.xlsx")
    test = pd.read_excel("study_program/test set.xlsx")



    knn_result = kNN(train,test)

    krai_list = []
    for i in range(len(knn_result.index)):
        temp_list = []
        for k in knn_result.iloc[i,1].split(', '): 
            temp_list.append(k)
            temp_list.append(knn_result.iloc[i,0])
            krai_list.append(temp_list)
    Rating_Matrix = pd.read_csv("study_program/Rating_Matrix.csv")

    program_name = []
    for i in range(len(Rating_Matrix.index)):
        program_name.append(Rating_Matrix.iloc[i,0])


    new_list = []
    for program in program_name:
        temp_name_list = []
        for item in krai_list:
            for i in range(0, len(item),2):
                #print("AE: ",item[i+1],program)
                if(item[i] == program):
                    if item[i+1] not in temp_name_list:
                        temp_name_list.append(item[i+1])
                    break
        new_list.append([program, temp_name_list])
    ...
    for item in new_list:
        if(len(item[1]) == 0):
            ...
            item[1] = random.sample(set(item[1]), 2)
            #print("AES: ", aes)

    #knn_recommend_list = new_list

    for item in new_list:
        knn_recommend_list.append([item[0],item[1][0], item[1][1]])

    return knn_recommend_list
...
def combine_recommendation(request):
    ...
    recommendation_committee_pearson_list = viewAllRecommendedCommittee(recommended_committee_df) #Pearson
    recommendation_committee_knn_list = viewAllNewRecomendedCommittee(1) #KNN
